import { normalizeKuwaitArabic } from "@/lib/ai/mobile-chat-kuwait-ar";
...
- اقتراح محتوى منشور (مناقصات، ممارسات/عطاءات في الجريدة، مراسيم، استدراكات، أخبار) حسب التبويب أو الوزارة أو النشاط عند توفير القائمة في السياق — لا ترفض هالأسئلة.
- «ممارسة» و«ممارسات» في لهجة المستخدم تعني عادةً مناقصة/عطاء منشور — اذكر من القائمة أو وجّه لتبويب المناقصات، لا ترفض.
- إجابات مختصرة وبلهجة كويتية واضحة (افهم: شنو، ايه، ابي، اسجل، ممارسة، عطاء، الحين، معروضة).
...
const IN_SCOPE_PATTERNS = [
  ...
  /\bpractice(s)?\b/i,
  /اسجل|سجّ?ل|تقديم/i,
  /شنو|ايش|قولي|علمني|دلني|ابي|ابغى|أبي|أبغى/iu,
  /عندكم|عندك|موجود/iu,
  /الحين|هس[ةه]|جديد/iu,
...
export function isOutOfScopeQuestion(text: string): boolean {
  ...
  if (IN_SCOPE_PATTERNS.some((p) => p.test(t) || p.test(n))) return false;
  return OFF_TOPIC_PATTERNS.some((p) => p.test(t) || p.test(n));
}
